...
function showTransformForm() {
  ...
  increaseContrastForm.addEventListener("submit", (e) => {
    ...
    increaseContrast(Number(x1),Number(x2),Number(y1),Number(y2))
  });
  ...
  decreaseContrastForm.addEventListener("submit", (e) => {
    ...
    var y1 = document.getElementById("y1").value
    var y2 = document.getElementById("y2").value
    decreaseContrast(Number(x1),Number(x2),Number(y1),Number(y2))
  });
  ...
  function increaseContrast(x1,x2,y1,y2) {
    const img = document.getElementById("inputImage");
    const canvas = document.getElementById("resultImage");
    const ctx = canvas.getContext('2d');
    ...
    for (i = 0; i < img.width * img.height * 4; i += 4) {
      ...
      c2=-(slope2*x2)+y2;
      c3=-(slope3*255)+255;
    if(0<=rgba[i] && rgba[i] <x1){
      val=(slope1*rgba[i])+c1;
    }

    else if(x1<=rgba[i] &&rgba[i] <x2){
      val=(slope2*rgba[i])+c2;
    }
    else{
      val=(slope3*rgba[i])+c3;
    }
      if (val > 255) {
        val = 255;
      }
  ...
  function decreaseContrast(x1,x2,y1,y2) {
    const img = document.getElementById("inputImage");
    const canvas = document.getElementById("resultImage");
    const ctx = canvas.getContext('2d');
